class UsersController < ApplicationController
    ...
    def set_admin_level
        ...
        return json_response({errors: error_desc2}, 401) if @current_user.admin_level == 3
    ...
    def suspend_communication
        ...
        return json_response({errors: error_desc}, 401) if @current_user.admin < 1
    ...
    def suspend_comms(user, comms, attr)
        ...
        d = DateTime.now
        ban_date = DateTime.now(comms_i[0], comms_i[1], comms_i[2], comms_i[3], comms_i[4], 0, d.offset);
        user.update_attribute(attr, ban_date)
    end

    def user_with_image(user)
        user_with_attachment = user.as_json(only: %i[id username is_activated admin_level can_post_date can_comment_date])
